...
e_samples = GN_BBH.generate_random_merger_eccentricities(n=N_samples,max_bh_mass=50)
...
rate_gn = 2.0  # Formation rate: 2 systems per Myr
print(f"==Generating snapshot for Steady State Population with Replenishing Rate = {rate_gn}/Myr==")
print(f"This corresponds to a GN BBH Meger Rate = {0.1*rate_gn:.3f}/Myr, or ~ {0.6*rate_gn:.3f} Gpc^-3 yr^-1 for MW-like galaxies in the Universe")
...
N_ync_sys = 100   # 100 massive BBHs formed in this cluster
...
snapshot_data = GN_BBH.generate_snapshot_population(Gamma_rep=rate_gn, ync_age=age_ync, ync_count=N_ync_sys,max_bh_mass=50)
...
f_peak_curr = calc_fpeak(9.227, 7.620, 1.633460e+01, 9.989424e-01)
print(f_peak_curr)