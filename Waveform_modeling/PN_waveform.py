...
def eccGW_waveform(f00, e0, timescale, m1, m2, theta, phi, R, l0=0, ts=None, PN_orbit=3, PN_reaction=2,
    ...
    CACHE_FILE = 'eccGW_1p5PN_table.npz'
    table_loaded = False
    kE_vals = None
    kJ_vals = None
    e_grid = None
...
def inner_product(fs, wavefrom1, waveform2, phic, snf=None):
    num1 = len(wavefrom1)
    dt = 1.0 / fs
    tobs = num1 * dt
    ...
    # 1. FFT 处理
    hf_1 = scipy.fftpack.fft(wavefrom1)
...
if __name__ == '__main__':
    ...
    print('SNR', SNR(m1,m2,a0,e0,Dl*1e3*pc,Tobs),SNR_approx(m1,m2,a0,e0,Dl*1e3*pc,Tobs))

    print('tmerger (yr)',tmerger_integral(m1,m2,a0,e0)/years,tmerger_lower(m1,m2,a0,e0)/years)
    ...
    c=time.time()
    SNR2=np.sqrt(inner_product.run(1/(hn1[0][1]-hn1[0][0]),hn1[1],hn1[1],0))
